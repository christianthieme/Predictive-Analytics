...
x <- select(df3, -brand_code)

df3x <- predict(pp, x)

df3x$brand_code <- df3$brand_code
...
xx <- select(eval2, -brand_code)

eval_x <- predict(ppe, xx)

eval_x$brand_code <- eval2$brand_code
...
eval2$pred <- predict(cubist, eval2)

# write.csv(eval2, 'c:/downloads\eval2_v2.csv')

ggplot(eval2, aes(x = pred )) +
  geom_density()
