...
Below is a plot of Var05 and Var07 from S03. The vertical lines represent the points where there is missing data. In this case, we `tidyr::fill()` to fill the missing value with the previous value in the dataset.You can see that by choosing this method, we haven't interfered with the natural trend of the data.
...
In several of the datasets, there are clear outliers that would strongly influence the models we built. Our approach was to use the `forecast::tsclean` function to resolve this issue. This function blah blah blah...
